...
struct stm32_priv_s
{
    ...
    struct ieee802154_radio_s * radio_lst[MAX_CC2520_CHIP];
                             ...
                             bool state)
{
  ...
  if (state)
    {
      stm32l4_gpiosetevent(priv->intcfg, false, true, true,
...
void stm32_irqworker(FAR void *arg)
{
    ...
    for(int minor = 0; minor < MAX_CC2520_CHIP; minor++){
...
static int stm32_cc2520_devsetup(struct stm32_priv_s *priv)
{
    ...
    up_udelay(200);
    stm32l4_gpiowrite(priv->rstcfg, 1);
