...
/* This structure describes the state of one FT6x06 driver instance */
...
struct ft6x06_dev_s
{
  ...
  volatile bool valid;                      /* True:  New, valid touch data
                                             * in touchbuf[] */
#ifdef CONFIG_FT6X06_SINGLEPOINT
...
#ifdef CONFIG_FT6X06_POLLMODE
  uint32_t delay;                           /* Current poll delay */
#endif

  FAR const struct ft6x06_config_s *config; /* Board configuration data */
...
#endif
  uint8_t touchbuf[FT6X06_TOUCH_DATA_LEN];  /* Raw touch data */
  ...
  struct pollfd *fds[CONFIG_FT6X06_NPOLLWAITERS];
};
...
static void ft6x06_notify(FAR struct ft6x06_dev_s *priv);
static void ft6x06_data_worker(FAR void *arg);
#ifdef CONFIG_FT6X06_POLLMODE
static void ft6x06_poll_timeout(wdparm_t arg);
#else
static int  ft6x06_data_interrupt(int irq, FAR void *context, FAR void *arg);
#endif
static ssize_t ft6x06_sample(FAR struct ft6x06_dev_s *priv, FAR char *buffer,
                             size_t len);
static ssize_t ft6x06_waitsample(FAR struct ft6x06_dev_s *priv,
                                 FAR char *buffer, size_t len);
static int  ft6x06_bringup(FAR struct ft6x06_dev_s *priv);
static void ft6x06_shutdown(FAR struct ft6x06_dev_s *priv);

/* Character driver methods */

static int  ft6x06_open(FAR struct file *filep);
static int  ft6x06_close(FAR struct file *filep);
static ssize_t ft6x06_read(FAR struct file *filep, FAR char *buffer,
                           size_t len);
static int  ft6x06_ioctl(FAR struct file *filep, int cmd,
                         unsigned long arg);
static int  ft6x06_poll(FAR struct file *filep, struct pollfd *fds,
                        bool setup);
...
static const struct file_operations ft6x06_fops =
{
  ft6x06_open,    /* open */
  ft6x06_close,   /* close */
  ft6x06_read,    /* read */
...
/* Maps FT6x06 touch events into bit encoded representation used by NuttX */

static const uint8_t g_event_map[4] =
{
  (TOUCH_DOWN | TOUCH_ID_VALID | TOUCH_POS_VALID), /* FT6X06_DOWN */
  (TOUCH_UP   | TOUCH_ID_VALID),                   /* FT6X06_UP */
  (TOUCH_MOVE | TOUCH_ID_VALID | TOUCH_POS_VALID), /* FT6X06_CONTACT */
  TOUCH_ID_VALID                                   /* FT6X06_INVALID */
};
...
static void ft6x06_notify(FAR struct ft6x06_dev_s *priv)
{
  /* If there are threads waiting on poll() for FT6x06 data to become
   * available, then wake them up now.  NOTE: we wake up all waiting threads
   * because we do not know that they are going to do.  If they all try to
   * read the data, then some make end up blocking after all.
   */

  poll_notify(priv->fds, CONFIG_FT6X06_NPOLLWAITERS, POLLIN);
  ...
  if (priv->nwaiters > 0)
    {
      /* After posting this semaphore, we need to exit because the FT6x06
...
/****************************************************************************
 * Name: ft6x06_data_worker
 ****************************************************************************/

static void ft6x06_data_worker(FAR void *arg)
{
  FAR struct ft6x06_dev_s *priv = (FAR struct ft6x06_dev_s *)arg;
  FAR const struct ft6x06_config_s *config;
  FAR struct ft6x06_touch_data_s *sample;
  ...
  regaddr          = FT6X06_TOUCH_DATA_STARTREG;
   ...
   *
   * REVISIT:  If CONFIG_FT6X06_SINGLEPOINT is selected, could we not just
  ...
  msg[1].length    = FT6X06_TOUCH_DATA_LEN;

  ret = I2C_TRANSFER(priv->i2c, msg, 2);
  if (ret >= 0)
    {
      /* In polled mode, we may read invalid touch data.  If there is
       * no touch data, the FT6x06 returns all 0xff the very first time.
       * After that, it returns the same old stale data when there is
       * no touch data.
       */

      sample = (FAR struct ft6x06_touch_data_s *)priv->touchbuf;

      /* Notify waiters (only if we ready some valid data).
       *
       * REVISIT: For improved performance consider moving the duplicate
       * report and thresholding logic from ft6x06_sample() to here.  That
       * would save a context switch.
       */

      if (sample->tdstatus <= FT6X06_MAX_TOUCHES)
        {
          /* Notify any waiters that new FT6x06 data is available */
      ...
      if (sample->tdstatus > 0 && sample->tdstatus <= FT6X06_MAX_TOUCHES)
        {
  ...
  wd_start(&priv->polltimer, priv->delay,
           ft6x06_poll_timeout, (wdparm_t)priv);

#else
  /* Exit, re-enabling FT6x06 interrupts */
...
static void ft6x06_poll_timeout(wdparm_t arg)
{
  ...
  /* Transfer processing to the worker thread.  Since FT6x06 poll timer is
   * disabled while the work is pending, no special action should be
   * required to protected the work queue.
   */

  DEBUGASSERT(priv->work.worker == NULL);
  ret = work_queue(HPWORK, &priv->work, ft6x06_data_worker, priv, 0);
  if (ret != 0)
    {
...
/****************************************************************************
 * Name: ft6x06_data_interrupt
 ****************************************************************************/

#ifndef CONFIG_FT6X06_POLLMODE
static int ft6x06_data_interrupt(int irq, FAR void *context, FAR void *arg)
{
  FAR struct ft6x06_dev_s *priv = (FAR struct ft6x06_dev_s *)arg;
  FAR const struct ft6x06_config_s *config;
  ...
  /* Transfer processing to the worker thread.  Since FT6x06 interrupts are
...
static ssize_t ft6x06_sample(FAR struct ft6x06_dev_s *priv, FAR char *buffer,
                             size_t len)
{
  FAR struct ft6x06_touch_data_s *raw;
  FAR struct ft6x06_touch_point_s *touch;
  ...
  raw = (FAR struct ft6x06_touch_data_s *)priv->touchbuf;
  touch = raw->touch;
  ...
  if (event == FT6X06_INVALID)
    {
      priv->lastevent = FT6X06_INVALID;
      goto reset_and_drop;
    }
  ...
  if (id == priv->lastid && event == priv->lastevent)
    {
      ...
      if (raw->tdstatus == 0 || event == FT6X06_UP)
        {
      ...
      else
        {
          ...
          if (deltax < CONFIG_FT6X06_THRESHX)
            {
                             ...
                             size_t len)
{
  ...
  DEBUGASSERT(ntouches <= FT6X06_MAX_TOUCHES);
...
#endif /* CONFIG_FT6X06_SINGLEPOINT */

/****************************************************************************
 * Name: ft6x06_waitsample
 ****************************************************************************/
  ...
  while (!priv->valid)
    {
      ...
      /* Wait for a change in the FT6x06 state */
  ...
  if (ret >= 0)
    {
      ...
      ret = ft6x06_sample(priv, buffer, len);
    }

errout:
  /* Restore pre-emption.  We might get suspended here but that is okay
   * because we already have our sample.  Note:  this means that if there
   * were two threads reading from the FT6x06 for some reason, the data
...
static int ft6x06_bringup(FAR struct ft6x06_dev_s *priv)
{
  ...
  data[0]       = FT6X06_TOUCH_MODE_REG;   /* Register address */
  data[1]       = FT6X06_DEV_MODE_WORKING; /* Normal mode */
...
#else
  FAR const struct ft6x06_config_s *config = priv->config;

  /* Make sure that the FT6x06 interrupt is disabled */
...
static int ft6x06_open(FAR struct file *filep)
{
  ...
  priv  = (FAR struct ft6x06_dev_s *)inode->i_private;
  ...
  if (tmp == 1)
    {
      ret = ft6x06_bringup(priv);
      if (ret < 0)
        {
          ierr("ERROR: ft6x06_bringup failed: %d\n", ret);
          goto errout_with_lock;
        }
    }
  ...
  while (ret == 0)
    {
      ...
      ret = ft6x06_waitsample(priv, buffer, len);
      if (ret < 0)
        {
...
static int ft6x06_ioctl(FAR struct file *filep, int cmd, unsigned long arg)
{
  FAR struct inode        *inode;
  FAR struct ft6x06_dev_s *priv;
  int                      ret;
...
static int ft6x06_poll(FAR struct file *filep, FAR struct pollfd *fds,
                       bool setup)
{
  FAR struct inode        *inode;
  FAR struct ft6x06_dev_s *priv;
  ...
  if (setup)
    {
      ...
      for (i = 0; i < CONFIG_FT6X06_NPOLLWAITERS; i++)
        {
      ...
      if (i >= CONFIG_FT6X06_NPOLLWAITERS)
        {
...
/****************************************************************************
 * Name: ft6x06_register
 *
 * Description:
 *   Configure the FT6x06 to use the provided I2C device instance.  This
...
int ft6x06_register(FAR struct i2c_master_s *i2c,
                    FAR const struct ft6x06_config_s *config, int minor)
{
  FAR struct ft6x06_dev_s *priv;
  ...
  /* Create and initialize a FT6x06 device driver instance */

  priv = (FAR struct ft6x06_dev_s *)kmm_zalloc(sizeof(struct ft6x06_dev_s));
  if (!priv)
    {
      ierr("ERROR: kmm_zalloc(%d) failed\n", sizeof(struct ft6x06_dev_s));
      return -ENOMEM;
    }

  /* Initialize the FT6x06 device driver instance */
...
#ifdef CONFIG_FT6X06_POLLMODE
  /* Allocate a timer for polling the FT6x06 */

  priv->delay     = POLL_MAXDELAY;
#else
  /* Make sure that the FT6x06 interrupt interrupt is disabled */
  ...
  ret = config->attach(config, ft6x06_data_interrupt,
                       priv);
  if (ret < 0)
    {
  ...
  ret = register_driver(devname, &ft6x06_fops, 0666, priv);
  if (ret < 0)
    {
